...
def calculate_path(src, dst):
    ...
    return pp.run_path_planning(step,
                                    start=(src[0], src[1]),
                                    finish=(dst[0], dst[1]), algo='Theta*', heur='euclidean',
                                    show_grid=True)
    ...
    for line in open(filepath):
        if re.match(r'^\(\w(\w|_)*(\ *(\w(\w|_)*)?)*\)', re.sub(r"(\d(\.|\d)*: ) *", "", line)):
            ...
            if tasks[-1].lower() == 'move':
                coords = split_line[2].replace('p', '').replace(')', '')
                tasks[-1] = (tasks[-1], (int(coords[1:3]), int(coords[3:])))
    print("TASKS:")
    print(tasks)
    return tasks
...
def visualize_paths_from_pddl(task_plan, path_to_map):
    position = (10, 4) # change this to the starting position for your PDDL problem
    ...
    for task in task_plan: # Iterate the whole task list
        ...
        if type(task) is tuple: # Movement must be a tuple, so we are only interested in this case
            if task[0].lower() == 'move': # This is the action we were looking for
...
task_plan = generate_task_list('planning.txt') # Must be a path to a text file
visualize_paths_from_pddl(task_plan, '../res/sandbox.png') # path_to_map_file should be a path to an image